...
import com.zq.asproj.model.Shop
 ...
 * 店铺模块= 基础信息+ 关联商品
 */
class ShopItem(val detailModel: DetailModel) : RVDataItem<DetailModel, RVViewHolder>() {
    ...
    override fun onBindData(holder: RVViewHolder, position: Int) {
        ...
        flowGoods?.let {
            val flowRecyclerView: RecyclerView = holder.findViewById(R.id.flow_recycler_view)!!
            ...
            if (flowRecyclerView.adapter == null) {
                flowRecyclerView.adapter = RVAdapter(context)
            }
            ...
            val adapter = flowRecyclerView.adapter as RVAdapter
            adapter.clearItems()
