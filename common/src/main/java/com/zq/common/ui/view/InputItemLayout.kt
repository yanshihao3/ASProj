...
class InputItemLayout @JvmOverloads constructor(
    ...
    init {
        ...
        val title = typedArray.getString(R.styleable.InputItemLayout_title)

        maxInputLength = typedArray.getInteger(R.styleable.InputItemLayout_maxInputLength, 20)

        //解析title资源属性
        parseTitleStyle(titleStyleId, title!!)

        //解析右侧的输入框属性
        val hint = typedArray.getString(R.styleable.InputItemLayout_hint)
        val inputResId =
            typedArray.getResourceId(R.styleable.InputItemLayout_inputTextAppearance, 0)
        val inputType = typedArray.getInteger(R.styleable.InputItemLayout_inputType, 0)
        parseInputStyle(hint, inputResId, inputType)
        //上下分割线属性
        val topResId = typedArray.getResourceId(R.styleable.InputItemLayout_topLineAppearance, 0)
        val bottomResId =
