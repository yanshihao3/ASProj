...
class HiRecyclerView @JvmOverloads constructor(
    ...
    inner class LoadMoreScrollListener(val prefetchSize: Int, val callback: () -> Unit) :
        OnScrollListener() {
        //咱们这里的强转，因为前面 会有前置检查
        val hiAdapter = adapter as RVAdapter
        ...
        private fun findLastVisibleItem(recyclerView: RecyclerView): Int {
            when (val layoutManager = recyclerView.layoutManager) {
                //layoutManager is GridLayoutManager 是 LinearLayoutManager的子类
    ...
    fun enableLoadMore(callback: () -> Unit, prefetchSize: Int) {
        if (adapter !is RVAdapter) {
            YLog.e("enableLoadMore must use hiadapter")
            return
        }
    ...
    fun disableLoadMore() {
        if (adapter !is RVAdapter) {
            YLog.e("disableLoadMore must use hiadapter")
            return
        }
    ...
    fun loadFinished(success: Boolean) {
        if (adapter !is RVAdapter) {
            YLog.e("loadFinished must use hiadapter")
            return
        }
